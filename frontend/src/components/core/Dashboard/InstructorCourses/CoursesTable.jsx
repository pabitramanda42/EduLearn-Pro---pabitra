...
export default function CoursesTable({
  ...
  // Skeleton row (must return <Tr><Td>... not <div>)
  const skItem = (key) => (
    <Tr key={key}>
      <Td colSpan={4}>
  ...
  return (
    <>
      <Table className="rounded-2xl border border-richblack-800 ">
        {/* heading */}
        <Thead>
          <Tr className="rounded-t-3xl border-b border-b-richblack-800 px-6 py-2">
        ...
        <Tbody>
          ...
          {!loading && courses?.length === 0 ? (
            ...
            courses?.map((course) => (
              <Tr
                key={course._id}
                className="border-b border-richblack-800 px-6 py-8"
              >
                <Td className="flex flex-1 gap-x-4 relative">
                  ...
                  <div className="flex flex-col">
                    ...
                    {course.status === COURSE_STATUS.DRAFT ? (
                      <p className="mt-2 flex w-fit items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-pink-100">
                        <HiClock size={14} />
                        Drafted
                      </p>
                    ) : (
                      <div className="mt-2 flex w-fit items-center gap-2 rounded-full bg-richblack-700 px-2 py-[2px] text-[12px] font-medium text-yellow-100">
